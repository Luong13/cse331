...
import { next_latin_char, prev_latin_char, count_consonants,
         pig_latin_encode, pig_latin_decode, cipher_encode, cipher_decode, crazy_caps_encode, crazy_caps_decode } from './latin_ops';
...
describe('char_list_ops', function() {
  ...
  it('cipher_encode', function() {
    // TODO: add tests
    const inputs = [explode("a"), explode("e"), explode("cd"), explode("ef")];
    const outputs = [explode("e"), explode("i"), explode("kt"), explode("iv")];

    //base case, zero recursive calls
    assert.deepEqual(cipher_encode(nil), nil);

    //2 tests for case of one recursive call
    assert.deepEqual(cipher_encode(inputs[0]), outputs[0]);
    assert.deepEqual(cipher_encode(inputs[1]), outputs[1]);

    //2 tests for case of many (2) recursive calls
    assert.deepEqual(cipher_encode(inputs[2]), outputs[2]);
    assert.deepEqual(cipher_encode(inputs[3]), outputs[3]);
  });

  it('cipher_decode', function() {
    // TODO: add tests
    const inputs = [explode("a"), explode("e"), explode("bp"), explode("ef")];
    const outputs = [explode("y"), explode("a"), explode("tb"), explode("aw")];

    //base case, zero recursive calls
    assert.deepEqual(cipher_decode(nil), nil);

    //2 tests for case of one recursive call
    assert.deepEqual(cipher_decode(inputs[0]), outputs[0]);
    assert.deepEqual(cipher_decode(inputs[1]), outputs[1]);

    //2 tests for case of many (2) recursive calls
    assert.deepEqual(cipher_decode(inputs[2]), outputs[2]);
    assert.deepEqual(cipher_decode(inputs[3]), outputs[3]);
  });

  it('crazy_caps_encode', function() {
    // TODO: add tests
    const inputs = [explode("a"), explode("bc"), explode("def"), explode("ghij"), explode("crazy"), explode("crazee")];
    const outputs = [explode("A"), explode("Bc"), explode("DeF"), explode("GhIj"), explode("CrAzY"), explode("CrAzEe")];

    //base case, zero recursive calls
    assert.deepEqual(crazy_caps_encode(nil), nil);

    //2 tests for case of zero recursive calls but not nil
    assert.deepEqual(crazy_caps_encode(inputs[0]), outputs[0]);
    assert.deepEqual(crazy_caps_encode(inputs[1]), outputs[1]);

    //2 tests for case of one recursive call
    assert.deepEqual(crazy_caps_encode(inputs[2]), outputs[2]);
    assert.deepEqual(crazy_caps_encode(inputs[3]), outputs[3]);

    //2 tests for case of many (2) recursive calls
    assert.deepEqual(crazy_caps_encode(inputs[4]), outputs[4]);
    assert.deepEqual(crazy_caps_encode(inputs[5]), outputs[5]);
  });
  ...
  it('crazy_caps_decode', function() {
    ...
    //base case, zero recursive calls
    assert.deepEqual(crazy_caps_decode(nil), nil);

    //2 tests for case of zero recursive calls but not nil
    assert.deepEqual(crazy_caps_decode(inputs[0]), outputs[0]);
    assert.deepEqual(crazy_caps_decode(inputs[1]), outputs[1]);

    //2 tests for case of one recursive call
    assert.deepEqual(crazy_caps_decode(inputs[2]), outputs[2]);
    assert.deepEqual(crazy_caps_decode(inputs[3]), outputs[3]);

    //2 tests for case of many (2) recursive calls
    assert.deepEqual(crazy_caps_decode(inputs[4]), outputs[4]);
    assert.deepEqual(crazy_caps_decode(inputs[5]), outputs[5]);
  });
