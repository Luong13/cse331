...
export function cipher_encode(L: List<number>): List<number> {
    ...
    const next : number = next_latin_char(L.hd);
    const result : List<number> = concat<number>(cons<number>(next, nil), cipher_encode(L.tl));
    return result;
}
export function cipher_decode(L: List<number>): List<number> {
    ...
    const prev : number = prev_latin_char(L.hd);
    const result : List<number> = concat<number>(cons<number>(prev, nil), cipher_decode(L.tl));
    return result;
}
...
export function crazy_caps_encode(L: List<number>): List<number> {
    ...
    const c : String = String.fromCharCode(L.hd);
    const c_up : String = c.toUpperCase();
    const c_num : number = c_up.charCodeAt(0);
    const c_list : List<number> = cons<number>(c_num, nil);

    if (L.tl === nil) {
        return c_list;
    }

    const recur : List<number> = crazy_caps_encode(L.tl.tl);
    const result = concat<number>(concat<number>(c_list, cons(L.tl.hd, nil)), recur);
    return result;
}
export function crazy_caps_decode(L: List<number>): List<number> {
    ...
    const c : String = String.fromCharCode(L.hd);
    const c_down : String = c.toLowerCase();
    const c_num : number = c_down.charCodeAt(0);
    const c_list : List<number> = cons<number>(c_num, nil)
    ...
    const recur : List<number> = crazy_caps_decode(L.tl.tl);
    const result = concat<number>(concat<number>(c_list, cons(L.tl.hd, nil)), recur);
    return result;
}
