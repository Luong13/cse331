 ...
 */
export function showColors(props: {clist: ColorList, text: string}): JSX.Element {
  const names = props.clist.findMatchingNames(props.text.toLowerCase());
  return <div>{compact_list(getColorCards(props.clist, names))}</div>
}

// Map a list of names into a list of HTML elements that display each color.
function getColorCards(clist: ColorList, names: List<string>): List<JSX.Element> {
  if (names === nil) {
    return nil;
  } else {
    const [bg, fg] = clist.getColorCss(names.hd.toLowerCase());
    return cons(
        <span className="color-border" key={names.hd}>
          <span className="color-card"
                style={{backgroundColor: bg, color: fg}}>{names.hd}</span>
        </span>,
        getColorCards(clist, names.tl));
  }
}
 ...
 */
export function showHighlights(props: {clist: ColorList, highlights: List<Highlight>}): JSX.Element {
  return <div>{compact_list(getHighlights(props.clist, props.highlights, 0))}</div>;
}

// Maps the list of highlights into a list of spans showing them.
function getHighlights(clist: ColorList, highlights: List<Highlight>, key: number): List<JSX.Element> {
  if (highlights === nil) {
    return nil;
  } else {
    const h = highlights.hd;
    const [bg, fg] = clist.getColorCss(h.color.toLowerCase());
    return cons(
        <span className="highlight" key={key}
              style={{backgroundColor: bg, color: fg}}>{h.text}</span>,
        getHighlights(clist, highlights.tl, key+1));
  }
}